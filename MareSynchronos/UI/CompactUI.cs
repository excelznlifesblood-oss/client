...
public partial class CompactUi : WindowMediatorSubscriberBase
{
    ...
    public CompactUi(ILogger<CompactUi> logger, UiSharedService uiShared, MareConfigService configService, ApiController apiController, PairManager pairManager,
        ...
        : base(logger, mediator, "###ShoninSyncMainUI", performanceCollectorService)
    {
        ...
        TitleBarButtons = new()
        {
            new TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Shonin Sync Settings");
                    ImGui.EndTooltip();
                }
            },
            new TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Shonin Sync Event Viewer");
                    ImGui.EndTooltip();
                }
            }
        };
        ...
        WindowName = $"Shonin Sync {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###ShoninSyncMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Shonin Sync " + ver.Major + "." + ver.Minor + "." + ver.Build + "###ShoninSyncMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Shonin Sync installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Shonin Sync up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
        ...
        if (!_ipcManager.Initialized)
        {
            ...
            UiSharedService.ColorTextWrapped($"One or more Plugins essential for Shonin Sync operation are unavailable. Enable or update following plugins:", ImGuiColors.DalamudRed);
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Shonin Sync server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Shonin Sync server is currently offline.",
            ...
            ServerState.NoAutoLogon => "This character has automatic login into Shonin Sync disabled. Press the connect button to connect to Shonin Sync.",
            _ => string.Empty
        };
    }
